...
export function TeamBottomNav({
  ...
  return (
    <nav className="fixed inset-x-3 bottom-[max(.75rem,env(safe-area-inset-bottom))] z-50 sm:hidden">
      <div className="mx-auto grid max-w-md grid-cols-4 rounded-[1.5rem] border border-white/10 bg-grass/95 p-1.5 shadow-float backdrop-blur-xl">
        {items.map((item) => {
          ...
          return (
            <Link
              ...
              className={`flex min-h-12 flex-col items-center justify-center gap-1 rounded-[1.1rem] py-1 text-[10px] font-bold transition active:scale-95 ${selected ? "bg-white text-graphite shadow-sm" : "text-slate-400"}`}
            >
