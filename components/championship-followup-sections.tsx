...
export function ChampionshipMatchesView({
  ...
  const canManage = canOperate && (championship.status === "published" || championship.status === "active");
    ...
    <ChampionshipFollowupShell teamSlug={teamSlug} returnTo={returnTo} championship={championship} currentSection="matches">
      <section aria-labelledby="matches-title">
        ...
        <form action={basePath} method="get" className="app-surface mt-4 p-4">
          ...
          <button type="submit" className="mt-4 min-h-11 rounded-xl bg-emerald-800 px-4 text-sm font-black text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-600 focus-visible:ring-offset-2">Aplicar filtros</button>
        </form>
        {page.items.length ? <div className="mt-4 grid gap-3 lg:grid-cols-2">{page.items.map((fixture) => (
          <article key={fixture.id} className="app-surface p-5">
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              ...
              {canManage ? <Link href={buildChampionshipFollowupUrl({ teamSlug, championshipId: championship.id, section: "matches", returnTo, extras: { ...extras, fixture: fixture.id } })} className="inline-flex min-h-11 items-center text-sm font-black text-slate-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-600">Gerenciar confronto</Link> : null}
            </div>
...
export function ChampionshipStandingsView({ teamId, teamSlug, timeZone, returnTo, championship, standings, knockoutPage, groupNumber, canConfigure, advanceContext }: BaseProps & { teamId: string; standings: ChampionshipFollowupStanding[]; knockoutPage: ChampionshipFollowupFixturePage | null; groupNumber: number; canConfigure: boolean; advanceContext: ChampionshipAdvanceContext | null }) {
  const canAdvance = canConfigure && (championship.status === "published" || championship.status === "active");
...
export function ChampionshipTeamsView({ teamId, teamSlug, returnTo, championship, participants, canConfigure }: Omit<BaseProps, "timeZone"> & { teamId: string; participants: ChampionshipFollowupParticipant[]; canConfigure: boolean }) {
  const canWithdraw = canConfigure && (championship.status === "published" || championship.status === "active");
  return <ChampionshipFollowupShell teamSlug={teamSlug} returnTo={returnTo} championship={championship} currentSection="teams"><section aria-labelledby="teams-title"><p className="app-kicker">Inscrições do campeonato</p><h2 id="teams-title" className="mt-1 text-2xl font-black">Equipes</h2><p className="mt-1 text-sm text-slate-500">Identidade preservada no momento da inscrição.</p>{participants.length ? <div className="mt-4 grid items-start gap-3 lg:grid-cols-2">{participants.map((participant) => <article key={participant.id} className="app-surface p-4"><div className="flex min-h-12 items-center gap-3"><InternalSquadBadge badgeKey={participant.snapshot_badge_key} color={participant.snapshot_color} className="size-11 shrink-0" /><div className="min-w-0 flex-1"><h3 className="truncate font-black">{participant.snapshot_name}</h3><p className="mt-1 text-xs text-slate-500">Seed #{participant.seed}{participant.group_number ? ` · Grupo ${String.fromCharCode(64 + participant.group_number)}` : ""}</p></div><span className={`rounded-full px-2 py-1 text-[10px] font-black ${participant.status === "active" ? "bg-emerald-50 text-emerald-800" : "bg-red-50 text-red-700"}`}>{participant.status === "active" ? "Ativa" : "Retirada"}</span></div>{canWithdraw && participant.status === "active" ? <WithdrawParticipantForm teamId={teamId} teamSlug={teamSlug} championshipId={championship.id} participantId={participant.id} /> : null}</article>)}</div> : <div className="app-surface mt-4 border-dashed p-8 text-center"><p className="font-black">Nenhuma equipe inscrita</p><p className="mt-1 text-sm text-slate-500">Volte à configuração para adicionar as equipes do campeonato.</p></div>}{!canConfigure ? <p className="mt-4 text-xs text-slate-500">Owner ou admin registra retirada de equipe. Seu acesso permanece somente para consulta.</p> : null}</section></ChampionshipFollowupShell>;
}
