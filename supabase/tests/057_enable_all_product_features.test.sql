...
select is((select count(*) from private.product_feature_keys()), 19::bigint,
  'catálogo contém as dezenove capacidades validadas');
...
select is((select flags_changed from activation_result), 95,
...
), 95::bigint, 'ativação global é auditada sem atribuir ator humano');

set local role authenticated;
select set_config('request.jwt.claim.sub','f2500000-0000-4000-8000-000000000001',true);
select is((select count(*) from public.team_feature_flags), 19::bigint,
...
select is((select count(*) from public.team_feature_flags
  where team_id = 'f2510000-0000-4000-8000-000000000003'
    and enabled
), 19::bigint, 'times criados após o rollout recebem o catálogo ativo');
