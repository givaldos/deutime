-- Correlação de callback por ID não secreto da tentativa.
-- A assinatura Twilio permanece obrigatória no Route Handler; a função é
-- acessível somente ao service_role e preserva a máquina monotônica existente.
...
comment on function public.record_notification_callback_by_attempt_id(
  ...
  'Registra callback assinado por tentativa não secreta, sem token na URL.';
