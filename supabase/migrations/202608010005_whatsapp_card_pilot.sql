-- Expande o claim unitário do Sandbox para a versão card_v1.
-- O template textual v1 continua aceito para compatibilidade e fallback.
...
begin
  ...
  where claimed.id = requested_outbox_id
    ...
    and claimed.template_version in ('v1', 'card_v1')
...
comment on function public.claim_notification_for_sandbox_pilot(
  uuid, uuid, text, integer
) is 'Claims one allowlisted text/card Sandbox intention; never scans the queue.';
