...
-- Wrapper para incidente: falha quando múltiplas partidas, preserva lógica legada de placar e confirmação
...
begin
  ...
  if not exists (select 1 from public.athletes athlete join public.event_attendance attendance on attendance.athlete_id = athlete.id and attendance.event_id = target_event.id and attendance.status = 'confirmed' where athlete.id = incident_athlete_id and athlete.team_id = target_event.team_id) then raise exception 'The athlete must be confirmed for this match' using errcode='55000'; end if;
  if incident_assist_athlete_id is not null and not exists (select 1 from public.athletes athlete join public.event_attendance attendance on attendance.athlete_id = athlete.id and attendance.event_id = target_event.id and attendance.status = 'confirmed' where athlete.id = incident_assist_athlete_id and athlete.team_id = target_event.team_id) then raise exception 'The assisting athlete must be confirmed for this match' using errcode='55000'; end if;
  select coalesce((select squad.name from public.event_squads squad where squad.event_id = target_event.id order by squad.sort_order, squad.created_at limit 1), 'Time A'), coalesce((select squad.name from public.event_squads squad where squad.event_id = target_event.id order by squad.sort_order, squad.created_at offset 1 limit 1), 'Time B') into default_side_a_label, default_side_b_label;
  ...
  if incident_kind = 'goal' then update public.match_reports report set side_a_score = report.side_a_score + case when incident_scoring_side = 1 then 1 else 0 end, side_b_score = report.side_b_score + case when incident_scoring_side = 2 then 1 else 0 end where report.event_id = target_event.id; end if;
