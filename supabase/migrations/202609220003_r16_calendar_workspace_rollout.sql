-- R16 / WP-R16-05 / CAL-05 — rollout e sonda agregada do calendário.
...
create or replace function public.set_calendar_workspace_rollout(
  requested_enabled boolean,
  requested_team_id uuid default null
)
returns table (teams_seen integer, flags_changed integer)
language plpgsql
security definer
set search_path = ''
...
begin
  if requested_enabled is null then
    raise exception 'Calendar workspace rollout state is required' using errcode = '22023';
  end if;
  ...
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended('set_calendar_workspace_rollout', 0)
  );
  ...
  loop
    ...
    if responsible_user_id is null then
      raise exception 'Team % has no active owner or admin', current_team.id using errcode = '42501';
    end if;

    if not exists (
      select 1 from public.team_feature_flags flag
      where flag.team_id = current_team.id
        and flag.feature = 'calendar_workspace'
        and flag.enabled = requested_enabled
    ) then
      insert into public.team_feature_flags(team_id,feature,enabled,updated_by)
      values (current_team.id,'calendar_workspace',requested_enabled,responsible_user_id)
      ...
      values (
        current_team.id,null,'feature_flag.changed','team_feature_flag','calendar_workspace',
        jsonb_build_object(
          'enabled',requested_enabled,
          'source','calendar_workspace_rollout',
...
create or replace function public.get_calendar_workspace_health(requested_team_id uuid)
returns table (
...
as $$
  select
    ...
    private.is_team_feature_enabled(team.id,'calendar_workspace'),
    ...
    (select count(*) from public.event_schedule_conflicts conflict
      where conflict.team_id=team.id and conflict.status='pending'),
    (select flag.updated_at from public.team_feature_flags flag
      where flag.team_id=team.id and flag.feature='calendar_workspace')
  from public.teams team
  where team.id=requested_team_id;
$$;

revoke all on function private.product_feature_keys() from public;
revoke all on function public.set_calendar_workspace_rollout(boolean,uuid)
  from public,anon,authenticated;
grant execute on function public.set_calendar_workspace_rollout(boolean,uuid)
  to service_role;
revoke all on function public.get_calendar_workspace_health(uuid)
  from public,anon,authenticated;
grant execute on function public.get_calendar_workspace_health(uuid)
  to service_role;

comment on function private.product_feature_keys() is
  'Catálogo das 21 capacidades validadas herdadas por novos times no rollout global.';
comment on function public.set_calendar_workspace_rollout(boolean,uuid) is
  'Ativa ou restaura de forma idempotente e auditada somente o calendário de gestão, globalmente ou em um time piloto.';
comment on function public.get_calendar_workspace_health(uuid) is
  'Sonda agregada do calendário, sem títulos, nomes, URLs, IDs esportivos ou dados pessoais.';
comment on function public.set_all_product_features(boolean) is
  'Ativa ou desativa transacionalmente as 21 capacidades validadas e os 6 controles globais.';
