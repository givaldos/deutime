...
import { isChampionshipsEnabled } from "./server";

describe("gate server-side de campeonatos", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("aceita a primeira leitura positiva sem consulta duplicada", async () => {
    ...
    await expect(isChampionshipsEnabled("team-id")).resolves.toBe(true);
    expect(mocks.isTeamFeatureEnabled).toHaveBeenCalledOnce();
    expect(mocks.info).not.toHaveBeenCalled();
  });

  it("recupera uma única leitura negativa transitória sem expor o time", async () => {
    ...
    expect(mocks.info).toHaveBeenCalledWith(
      "championship_feature_lookup.recovered",
    );
  });
  ...
  it("permanece desligado depois de duas leituras negativas", async () => {
    ...
    await expect(isChampionshipsEnabled("team-id")).resolves.toBe(false);
    expect(mocks.isTeamFeatureEnabled).toHaveBeenCalledTimes(2);
    expect(mocks.info).not.toHaveBeenCalled();
  });
});
