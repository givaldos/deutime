...
8. **Agenda profissional** — conforme [`DEC-PROFESSIONAL-SCHEDULING`](decisions/DEC-PROFESSIONAL-SCHEDULING.md), jogo continua sendo uma ocorrência, recorrência apenas materializa ocorrências e campeonato organiza confrontos. Duas equipes internas distintas podem preencher novos jogos, sem substituir snapshots. Regulamentos publicados são versionados; conflitos de equipe, local e atleta são projeções privadas revalidadas na escrita e resolvidas manualmente com auditoria. A flag `professional_scheduling` nasce desligada e a agenda atual permanece o fallback.
9. **Reconhecimento positivo** — conforme [`DEC-RECOGNITION-MODEL`](decisions/DEC-RECOGNITION-MODEL.md), `recognition-v1` projeta somente gol, assistência e resultado agregado fechado do Craque a partir de participação real em partida finalizada. Cada item pertence a `athlete_id + team_id`, acompanha correção ou reversão da fonte e não cria ledger, pontos ou ranking. A primeira ativação da flag `recognition` define o limite não retroativo; o resumo público exige `public_recognition_summary_v1` por vínculo e expõe somente categoria e contagem.
10. **Conversa da súmula** — conforme [`DEC-CONVERSATION-LIFETIME`](decisions/DEC-CONVERSATION-LIFETIME.md), comentários privados pertencem à partida, exigem identidade verificada e audiência congelada SIM/TALVEZ ou staff ativo; escrita fecha em sete dias e não cria chat geral.
11. **Comunicação** — `communication_consents` registra opt-in/opt-out e evidência; `notification_outbox` desacopla eventos do domínio do futuro provedor de WhatsApp.
12. **Auditoria** — `audit_logs` registra mudanças sensíveis de estado sem armazenar o conteúdo completo da PII.
