...
## `WP-R03R-03` — CP3 a CP6 concluídos
...
- em 2026-08-10, o primeiro lembrete do CP4 foi isolado no evento demo
  `d7f766f2-bb01-4db3-8703-062be9c6c640`: o pré-check confirmou a próxima cota
  `reminder_1`, três destinatários elegíveis, features e controles ligados,
  nenhuma cota vencida e nenhuma mensagem acionável concorrente;
- a solicitação auditada `5b89effd-b137-4265-8e1c-dc07a957c741` consumiu uma
  única vez a cota e criou 3/3 mensagens. O primeiro worker, run `31446736831`,
  fechou com `409 consumo desligado` antes de qualquer claim; a única repetição
  autorizada, run `31446795713`, terminou com 3 `claimed`, 3 `prepared` e 3
  `accepted`, sem `rejected`, `ambiguous` ou `cancelled`;
- a conferência persistida terminou com 3/3 outbox `sent` e
  `requires_review=0`. Esta é evidência parcial do CP4; `AC-R03R-09` continua
  aberta até confirmar card/fallback, link e RSVP no WhatsApp real em Android e
  iPhone e repetir o processo para `reminder_2`.
- o primeiro lembrete foi confirmado fisicamente em dois celulares e duas
  sessões autenticadas distintas, Neymar em um aparelho e Pelé no outro. Card
  e link chegaram corretamente e não houve mistura de identidade;
- o pré-check seguinte confirmou `reminder_2`, três pendentes elegíveis, fila
  vazia, nenhuma cota automática vencida e todos os controles ligados. A
  solicitação auditada `bcdfe37a-0d3b-48e5-867c-441261463c05` consumiu uma
  única vez a última cota e criou 3/3 mensagens;
- o worker run `31447333204` terminou com 3 `claimed`, 3 `prepared` e 3
  `accepted`, sem `rejected`, `ambiguous` ou `cancelled`. A conferência
  persistida terminou com 3/3 outbox `sent` e `requires_review=0`;
- a última chamada foi confirmada fisicamente nos dois aparelhos, com card,
  link, sessão e RSVP funcionando. Com convite, primeiro lembrete, última
  chamada e fallback já exercitados, `AC-R03R-09` foi concluída;
- `APP_URL=https://deutime.app npm run smoke:production` passou depois do
  piloto; o evento público opcional não está configurado;
- CP5 e CP6 estão concluídos: piloto, telemetria, fallback, contenção,
  rollback, critérios, documentação e rollout possuem evidência. R03R está
  concluída e o checkpoint voltou a `idle`.
