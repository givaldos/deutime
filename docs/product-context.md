...
- `DEC-EVENT-SURFACE-FOCUS` — `accepted`: a página aberta do evento concentra situação do jogo, divisão e chamada; compartilhar é uma ação única que envia contexto e URL em um bloco ordenado, com fallback de cópia. Configurações de lembretes automáticos e envio manual ficam recolhidas em Editar. Na divisão explicitamente publicada, somente o primeiro nome chega ao HTML e à imagem.
