...
- [`DEC-ACCOUNT-LIFECYCLE`](decisions/DEC-ACCOUNT-LIFECYCLE.md): saída e encerramento revogam acesso e projeções imediatamente, preservam somente fatos esportivos anonimizados e evidência mínima por prazo definido; o último owner transfere a propriedade ou encerra explicitamente o time.
...
- [`DEC-PROFESSIONAL-SCHEDULING`](decisions/DEC-PROFESSIONAL-SCHEDULING.md): jogo, recorrência, campeonato, equipe, escalação e partida permanecem conceitos distintos; duas equipes internas padrão preenchem novos jogos, regulamento publicado é versionado e conflitos de agenda exigem decisão humana auditável, nunca remarcação automática.
- [`DEC-SUBSCRIPTION-BILLING`](decisions/DEC-SUBSCRIPTION-BILLING.md) — `proposed`: R11 prefere checkout recorrente hospedado do Asaas atrás de adapter neutro; retorno do navegador nunca ativa benefício, webhooks são autenticados e idempotentes e o aceite aguarda ensaio real no Sandbox e políticas comerciais.
