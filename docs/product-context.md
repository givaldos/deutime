...
- `DEC-EVENT-SURFACE-FOCUS` — `accepted`: a página aberta do evento concentra situação do jogo, divisão e chamada; compartilhar é uma ação única que usa o compartilhamento nativo com fallback de cópia. Configurações de lembretes automáticos e envio manual ficam recolhidas em Editar.
